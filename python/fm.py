...
class FM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        w0 = 0.0
        w = np.zeros(size=(self.n_features))
            ...
            if self.regression:
                ...
                for ix, x in enumerate(self.X):
                    for i in range(self.n_features):
                        v_grad = loss[ix] * (x.dot(v).dot(x[i])[0] - v[i] * x[i] ** 2)
                        v[i] -= self.lr * v_grad + (2 * self.reg_v * v[i])
                        
            else:
                loss=np.sum(-np.log(expit(self.y*pred)))
                w0 -= self.lr*2*(pred-self.y) + 2*self.reg_w0*w0
