...
class Constant(Initializer):
    ...
    def __call__(self, shape, dtype=None):
        return np.full(shape, fill_value=self.value, dtype=dtype)
...
class RandomUniform(Initializer):
    ...
    def __call__(self, shape, dtype=np.float32):
        return np.random.uniform( self.minval, self.maxval,shape, size=shape)
...
class TruncatedNormal(Initializer):
    ...
    def __init__(self, mean=0., stddev=0.05, lower=-2.0, higher=2.0):
        self.mean = mean
        self.stddev = stddev
        self.lower=lower
        self.higher = higher
    ...
    def __call__(self, shape, dtype=np.float32):
        return np.array(truncnorm.rvs(self.lower, self.higher,
            loc=self.mean, scale=self.stddev, size=shape))
...
class VarianceScaling(Initializer):
    ...
    def __call__(self, shape, dtype=None):
        ...
        if self.distribution == 'normal':
            ...
            return np.array(truncnorm.rvs(-2.0, 2.0, loc=0., scale=stddev, size=shape))
        else:
...
def serialize(initializer):
    return serialize_object(initializer)
